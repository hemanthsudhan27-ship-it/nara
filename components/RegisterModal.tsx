...
export default function RegisterModal({
  ...
  const handleOpenWhatsAppDirect = async () => {
    ...
    const targetUrl = whatsappUrl || `https://wa.me/917907318843?text=${encodeURIComponent(textMessage)}`;

    // Direct redirect to +91 7907318843
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Open/redirect to WhatsApp directly (+91 7907318843)
      const targetUrl = data.whatsappUrl || `https://wa.me/917907318843?text=${encodeURIComponent(buildWhatsAppMessage(assignedId))}`;
      const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
      if (isMobile) {
        window.location.href = targetUrl;
      } else {
            ...
            <div className="text-center py-3 space-y-4">
              ...
              <div className="space-y-2 pt-1">
                ...
                >
                  ...
                  <span>Open WhatsApp Chat (+91 7907318843)</span>
                </button>
                <p className="text-[11px] text-neutral-300 bg-[#25D366]/10 border border-[#25D366]/30 p-2.5 rounded-xl text-left">
                  📲 <strong>Important:</strong> When WhatsApp opens with your booking message, please tap the attachment (📎 / 📷) icon in WhatsApp and send your payment screenshot to <strong className="text-white">+91 7907318843</strong> to complete verification.
                </p>
              </div>
